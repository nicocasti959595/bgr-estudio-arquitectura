...
// ============================================================
//  BANNERS / CARRUSEL DE PROMOS
// ============================================================

const BANNERS_LIMITE_SRV = 8;

export async function subirBannerAction(formData: FormData) {
  const supabase = await requireAdmin();
  const archivo = formData.get("archivo") as File | null;
  const titulo = String(formData.get("titulo") ?? "").trim();
  const link = String(formData.get("link") ?? "").trim();

  if (!archivo) return { ok: false as const, error: "No se recibió archivo." };
  ...
  if (archivo.size > 2.5 * 1024 * 1024) {
    return {
      ok: false as const,
      error: "El archivo supera los 2.5MB. Optimizalo y volvé a intentar.",
    };
  }
  ...
  const { count } = await supabase
    .from("bgr_banners")
    .select("*", { count: "exact", head: true });
  if ((count ?? 0) >= BANNERS_LIMITE_SRV) {
    return {
      ok: false as const,
      error: `Llegaste al límite de ${BANNERS_LIMITE_SRV} banners. Borrá uno primero.`,
    };
  }

  const ext = archivo.name.split(".").pop()?.toLowerCase() ?? "jpg";
  const nombre = `banners/${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 8)}.${ext}`;
  ...
  const { error: insErr } = await supabase.from("bgr_banners").insert([
    {
      imagen_url: publica.publicUrl,
      titulo: titulo || "Banner",
      link: link || null,
      orden: (ult?.orden ?? 0) + 1,
      activo: true,
    },
...
export async function actualizarBannerAction(
  id: string,
  campos: { titulo?: string; link?: string | null }
) {
  const supabase = await requireAdmin();
  const update: Record<string, unknown> = {
    updated_at: new Date().toISOString(),
  };
  if (campos.titulo !== undefined) update.titulo = campos.titulo.trim() || "Banner";
  if (campos.link !== undefined) update.link = campos.link?.trim() || null;
...
export async function toggleBannerActivoAction(id: string, activo: boolean) {
  const supabase = await requireAdmin();
  const { error } = await supabase
    .from("bgr_banners")
...
export async function moverBannerAction(id: string, direccion: "subir" | "bajar") {
  const supabase = await requireAdmin();
  const { data: todos } = await supabase
    .from("bgr_banners")
    .select("id, orden")
    .order("orden", { ascending: true });
  if (!todos) return { ok: false as const, error: "No se pudo leer." };

  const idx = todos.findIndex((b) => b.id === id);
  if (idx === -1) return { ok: false as const, error: "Banner no encontrado." };
  const swapIdx = direccion === "subir" ? idx - 1 : idx + 1;
  if (swapIdx < 0 || swapIdx >= todos.length) return { ok: true as const };

  const a = todos[idx];
  const b = todos[swapIdx];
  await supabase
    .from("bgr_banners")
    .update({ orden: b.orden, updated_at: new Date().toISOString() })
    .eq("id", a.id);
  await supabase
    .from("bgr_banners")
    .update({ orden: a.orden, updated_at: new Date().toISOString() })
    .eq("id", b.id);
...
export async function eliminarBannerAction(id: string) {
  const supabase = await requireAdmin();
  const { error } = await supabase.from("bgr_banners").delete().eq("id", id);
  if (error) return { ok: false as const, error: error.message };
  revalidatePath("/");
  revalidatePath("/admin/promos");
  return { ok: true as const };
