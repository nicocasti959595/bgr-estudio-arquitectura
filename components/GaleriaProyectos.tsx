...
// Fallback con obras de ejemplo (se muestra si la BD aún no tiene proyectos cargados)
const itemsFallback: Array<
  Pick<Proyecto, "tipo" | "titulo" | "barrio" | "imagen_portada" | "instagram_url">
> = [
  {
...
export async function GaleriaProyectos() {
  const proyectosBD = await getProyectos();
  const items = proyectosBD.length > 0 ? proyectosBD : itemsFallback;
  const linkPorDefecto = "https://instagram.com/bgr.construcciones";

  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-[2px]">
      {items.map((p, i) => {
        const href = p.instagram_url || linkPorDefecto;
        return (
          <a
            key={
              "id" in p && p.id ? String(p.id) : `${p.titulo}-${p.barrio ?? i}`
            }
            href={href}
            target="_blank"
